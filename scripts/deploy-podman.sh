...
echo ------------ 1 ------------
podman-compose -f podman-compose.prod.yml build $COMPOSE_OPTS lutrisweb

echo ------------ 2 ------------
podman-compose -f podman-compose.prod.yml build $COMPOSE_OPTS lutrisworker

echo ------------ 3 ------------
podman-compose -f podman-compose.prod.yml build $COMPOSE_OPTS lutrisnginx
...
if [[ "$3" == "--merge" ]] || [[ "$4" == "--merge" ]]; then
    echo ------------ a ------------
    podman-compose -f podman-compose.prod.yml exec lutrisweb ./manage.py makemigrations --merge

    echo ------------ b ------------
    podman-compose -f podman-compose.prod.yml exec lutrisweb ./manage.py migrate
fi

echo "Restarting NGinx"
echo ------------ 5 ------------
podman-compose -f podman-compose.prod.yml restart lutrisnginx
